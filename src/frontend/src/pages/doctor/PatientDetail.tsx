...
import { useGetPatientProfile, useGetVitalsReadings, useGetAlerts, useGetMedicationReminders, useGetAppointments, useGetHomeNurseRequests } from '../../hooks/useQueries';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import VitalsList from '../../components/vitals/VitalsList';
import { Badge } from '@/components/ui/badge';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Activity, AlertCircle, Pill, Calendar, Heart } from 'lucide-react';
import { Variant_emergency_medication_vitals } from '../../backend';

export default function PatientDetail() {
  const { patientId: patientIdStr } = useParams({ from: '/authenticated/doctor/patient/$patientId' });
  ...
  const { data: nurseRequests = [] } = useGetHomeNurseRequests(patientId);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">{profile?.name || 'Patient Details'}</h1>
        <p className="text-muted-foreground">Comprehensive patient health overview</p>
      </div>
      ...
      <Tabs defaultValue="vitals" className="space-y-6">
        ...
        <TabsContent value="medications">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Patient's current medications</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="nurse">
          <Card>
            <CardHeader>
              <CardTitle>Home Nurse Requests</CardTitle>
              <CardDescription>Patient's home care requests</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <TableBody>
                    {nurseRequests.map((req) => (
                      <TableRow key={req.id.toString()}>
                        <TableCell className="font-medium">{formatDate(req.dateTime)}</TableCell>
